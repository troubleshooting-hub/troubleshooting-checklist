...
const binList = document.getElementById("binIssueList");
...
function renderBinList() {
  ...
  if (binCountPillTab) {
    binCountPillTab.textContent = String(deletedIssues.length);
  }
      ...
      <div class="kv-val">
        <ul class="bullet-list">
          ${variants
            .map(
              (v) =>
                `<li><button type="button" class="btn btn-secondary small" data-suggest-issue="${escapeHtml(v.issueDescription || "")}">${escapeHtml(v.issueDescription || "")}</button></li>`
            )
            .join("")}
        </ul>
...
function renderHelpOutput({ matchedIssue, missingItems, userIssueText }) {
  ...
  const variantsHint =
    normalizeForCompare(userIssueText).trim() === "409" ||
    normalizeForCompare(userIssueText).startsWith("409 ")
...
function bindEvents() {
  ...
  helpClearBtn?.addEventListener("click", () => {
    ...
    if (helpResults) {
      helpResults.innerHTML = `<div class="muted">Enter an issue above, paste what you checked, and click “Analyze my checklist”.</div>`;
    }
